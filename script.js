...
let totalUnits = 15; // 시작 유닛은 레벨 15부터 15로 가정합니다.
let winStreak = 0; // 연속 승리 카운터
...
const levelMap = {
    ...
    30: {
        ...
        // 예를 들어: 1: { bet: 10, win: { type: 'gameWin' }, lose: { type: 'gameOver' } }
    }
};
...
function initializeGame() {
    currentLevel = 15; // 시작 레벨 15로 변경
    currentStage = 1;
    totalUnits = 15; // 시작 유닛 15로 변경
    winStreak = 0;
    betHistory = [];
    updateDisplay();
...
function updateDisplay() {
    ...
    if (totalUnits <= 0 && !(currentLevel === 15 && currentStage === 1 && totalUnits === 15)) {
...
function handleLose() {
    ...
    // 레벨 22, 23, 24, 25, 26의 4단계 패배 로직
        ...
        if (lostBetUnit === stageData.bet1) { // 1번째 베팅에서 패배
            nextLevel = currentLevel - 15;
            messageEl.textContent = `패배! (1번째 베팅 실패) 레벨 ${nextLevel}로 이동합니다.`;
        } else if (lostBetUnit === stageData.bet2) { // 2번째 베팅에서 패배
